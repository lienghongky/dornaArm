...
class arm:
	...
	self.DEFAULT_CONFIG = "home/pi/Desktop/config.yaml"
	...
	self.JOINTS = ['j0','j1','j2','j3','j4']

	# Utility poses

	self.HOME_POSE = pose([0,135,-90,-45,0],"the HOME (resting) position")
	self.FLAT_POSE = pose([0,0,0,0,0],"the FLAT (outstretched) position")
	...
	def __init__(self,port=None,config=None):
		...
		try:
			return self.robot.connect(port)
		except:
	...
	def goToPose(pose,speed=self.DEFAULT_SPEED):
		...
		self.robot.play(getMovementCommand(self.HOME_POSE,speed=speed))
		print("Completed move")

	def getMovementCommand(pose,space='joint',movement=self.ABSOLUTE,speed=self.DEFAULT_SPEED):
